...
- **How:** VLM primary — four sequential focused single-region calls, one per auxiliary, with plan_view DERIVED from the page rect minus the auxiliaries. The four calls run in order: `vlm.detect_title_block`, `vlm.detect_legend`, `vlm.detect_notes`, `vlm.detect_schedule`. Each returns its own normalized bboxes (single-bbox for title / schedule, multi-bbox for legend / notes) via a single-question, schema-only, 30-50-token prompt — the model isn't asked to disambiguate region kinds, just to localise one type at a time. Calls are independent: a `VLMError` on any one call logs at WARNING and skips that region; the others still run. Empty / null results are non-failure ("no such region on this drawing"). plan_view is derived by starting with the full page rect and, for each identified auxiliary, identifying its nearest page edge (top / bottom / left / right by minimum distance to the page edge) and clipping plan_view's edge to the auxiliary's inner edge minus a 1% safety pad. Multiple auxiliaries on the same edge collapse naturally — the deepest clip wins. Any single-edge clip that would exceed 35% of the page dimension is rejected with a WARNING (no real auxiliary takes a third of a page). The derived plan_view runs through the same plausibility guard as before (area outside [1%, 99%], or page-bounds within 5% slop) → on failure we fall through to the heuristic. The auxiliary regions we successfully identified are surfaced into the layout (`title_block`, `legend`, `notes`, `schedule`) with the same 3% padding as before. The heuristic fallback (Hough-line decomposition + OCR keyword match against `LEGEND`/`NOTES`/`SCHEDULE`/`PLAN`/`LEVEL`/`FLOOR`/`LAYOUT`/`HVAC`, plus per-rectangle `categorize_region` for unclassified rects) still runs when the derived plan_view fails the plausibility guard.

  **Iteration history of the VLM-first path:** (1) Single combined `detect_page_regions` call returning all five regions in one schema — the model conflated plan_view with title bars, swapped legend↔notes, and over-bounded plan_view to whole-page. (2) Two focused calls: `detect_plan_view` first, `detect_legend` second; plan_view directly asked. The plan_view call still over-bounded — "all the duct stuff but NOT the title banner / legend / notes" is hard to define positively. (3) THIS revision: ask only for auxiliaries (each has a distinct visual shape — banner-shaped title, table-shaped legend, text-block-shaped notes), then derive plan_view geometrically as the page complement.

  **Model-swap follow-up:** all three revisions above were driven by llama3.2-vision's small-model failure modes (11B parameters, weak at "where is X on this page" without tight, single-question prompts). With the default VLM swapped to `qwen3-vl:235b-cloud` (~20× larger and purpose-built for vision), several of these workarounds — multi-call decomposition, schema-only prompts, geometric plan_view derivation — may be reducible or collapsible back to the simpler combined call. The current pipeline keeps the auxiliary-first path as-is for now (it works on the smaller model and remains correct on the larger one); tuning for qwen3-vl is a follow-up, not part of the swap itself. Per-VLM prompt directories (`prompts/qwen3-vl/...` vs `prompts/llama3.2-vision/...`) let each model evolve independently.
...
- **V2 ships with `qwen3-vl:235b-cloud` as the default VLM (Ollama Cloud);** llama3.2-vision is a valid local override. ADR-0002's pluggable `VLMClient` seam means swapping to Claude or other providers is a config + prompt-directory change, not a rewrite. Per-VLM prompt directories (`prompts/{model_slug}/...`) let each model carry its own tuned prompts without cross-model regression risk.
...
- **VLM is the ceiling.** The system is bounded by what the configured vision model can do. v2 ships with `qwen3-vl:235b-cloud` (Ollama Cloud) as the default; `llama3.2-vision` (local, 11B) remains a valid override. Other providers (Claude vision, GPT-4V) plug in via ADR-0002's `VLMClient` seam — config + a per-VLM prompt directory.
