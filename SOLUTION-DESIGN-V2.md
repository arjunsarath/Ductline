...
- **How:** VLM primary — a single whole-page `vlm.detect_page_regions(raster_probe)` call returns rough normalized bboxes (plan_view, legend, schedule, title_block, notes) via the typed `PageRegionsTool` schema. Heuristic fallback (Hough-line decomposition + OCR keyword match against `LEGEND`/`NOTES`/`SCHEDULE`/`PLAN`/`LEVEL`/`FLOOR`/`LAYOUT`/`HVAC`, plus per-rectangle `categorize_region` for unclassified rects) fires only when the VLM call errors or its output fails the soft plausibility guard (plan_view missing, area outside [1%, 99%], or essentially the whole page).
