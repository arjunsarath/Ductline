...
- **How:** Algorithmic primary — long horizontal/vertical Hough lines split the sheet into rectangles; each rectangle is classified by text-density + OCR keyword match (`LEGEND`, `NOTES`, `SCHEDULE`, `PLAN`, `LEVEL`, `FLOOR`). VLM fallback for unclassified rectangles via a typed `CategorizePageTool` schema.
