...
def test_v3_drawing_03_endtoend(picks_03: V3PipelineConfig) -> None:
    ...
    assert result.target_dpi >= 400, "expected adaptive DPI to land >= 400 for vector PDF with small text"
    assert result.rotation_applied in (0, 90, 180, 270)
    assert result.page_unit == "in", "drawing 03 has 41 CFM tokens, 0 L/s — page unit must resolve to 'in'"
    ...
    assert 35 <= len(result.segments) <= 60, (
